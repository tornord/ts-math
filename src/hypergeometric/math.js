...
function hypergeometric2F1( a, b, c, x, tolerance=1e-10, pfaffLimit = null) {
  ...
  if ( isComplex(a) || isComplex(b) || isComplex(c) || isComplex(x) ) {
    ...
    switch( index ) {
      ...
      case 1:
        ...
        return mul( pow( sub(1,x), neg(a) ), hypergeometric2F1( a, sub(c,b), c, div(x,sub(x,1)), tolerance, pfaffLimit ) );

      case 2:

        if ( isInteger(sub(c,add(a,b))) || isNegativeIntegerOrZero(sub(c,a)) )
          return complexAverage( a => hypergeometric2F1(a,b,c,x, tolerance, pfaffLimit), a );

        if ( isNegativeIntegerOrZero(sub(c,b)) )
          return complexAverage( b => hypergeometric2F1(a,b,c,x, tolerance, pfaffLimit), b );

        var t1 = mul( gamma(c), gamma( sub(c,add(a,b)) ), 
                      inv( gamma(sub(c,a)) ), inv( gamma(sub(c,b)) ),
                      hypergeometric2F1( a, b, add(a,b,neg(c),1), sub(1,x), tolerance, pfaffLimit ) );

        var t2 = mul( pow( sub(1,x), sub(c,add(a,b)) ),
                      gamma(c), gamma( sub(add(a,b),c) ), inv( gamma(a) ), inv( gamma(b) ),
                      hypergeometric2F1( sub(c,a), sub(c,b), add(c,neg(a),neg(b),1), sub(1,x), tolerance, pfaffLimit ) );
      ...
      case 3:
        ...
        var t1 = mul( gamma(c), gamma(sub(b,a)), inv( gamma(b) ),
                      ...
                      hypergeometric2F1( a, add(1,neg(c),a), add(1,neg(b),a), inv(x), tolerance, pfaffLimit ) );

        var t2 = mul( gamma(c), gamma(sub(a,b)), inv( gamma(a) ),
                      inv( gamma(sub(c,b)) ), pow( neg(x), neg(b) ),
                      hypergeometric2F1( b, add(1,neg(c),b), add(1,neg(a),b), inv(x), tolerance, pfaffLimit ) );
      ...
      case 4:
        ...
        var t1 = mul( pow( sub(1,x), neg(a) ), gamma(c), gamma(sub(b,a)),
                      ...
                      hypergeometric2F1( a, sub(c,b), add(a,neg(b),1), inv(sub(1,x)), tolerance, pfaffLimit ) );

        var t2 = mul( pow( sub(1,x), neg(b) ), gamma(c), gamma(sub(a,b)),
                      inv( gamma(a) ), inv( gamma(sub(c,b)) ),
                      hypergeometric2F1( b, sub(c,a), add(b,neg(a),1), inv(sub(1,x)), tolerance, pfaffLimit ) );
      ...
      case 5:
        ...
        var t1 = mul( gamma(c), gamma( sub(c,add(a,b)) ), inv( gamma(sub(c,a)) ),
                      ...
                      hypergeometric2F1( a, add(a,neg(c),1), add(a,b,neg(c),1), sub(1,inv(x)), tolerance, pfaffLimit ) );

        var t2 = mul( gamma(c), gamma( sub(add(a,b),c) ), inv( gamma(a) ), inv( gamma(b) ),
                      pow( sub(1,x), sub(c,add(a,b)) ), pow( x, sub(a,c) ),
                      hypergeometric2F1( sub(c,a), sub(1,a), add(c,neg(a),neg(b),1), sub(1,inv(x)), tolerance, pfaffLimit ) );
    ...
    if (i>=100) console.log(`Warning, more than 100 iterations, x = ${x}, iterations = ${i}`)
    ...
    if ( x < -1 ) {
      ...
      var t1 = gamma(c) * gamma(b-a) / gamma(b) / gamma(c-a)
                 * (-x)**(-a) * hypergeometric2F1( a, 1-c+a, 1-b+a, 1/x, tolerance, pfaffLimit );
      var t2 = gamma(c) * gamma(a-b) / gamma(a) / gamma(c-b)
                 * (-x)**(-b) * hypergeometric2F1( b, 1-c+b, 1-a+b, 1/x, tolerance, pfaffLimit );

      return t1 + t2;

    }

    // const pfaffLimit = -0.8;
    if ( x === -1 || (pfaffLimit && x <= pfaffLimit && x > -1)) return hypergeometric2F1( a, b, c, complex(x), tolerance, pfaffLimit ).re;

    if ( x === 1 )
      if ( c - a - b > 0 ) return gamma(c) * gamma(c-a-b) / gamma(c-a) / gamma(c-b);
      else throw Error( 'Divergent Gauss hypergeometric function' );

    if ( x > 1 ) return hypergeometric2F1( a, b, c, complex(x), tolerance, pfaffLimit );
...
module.exports = { hypergeometric2F1, gamma, complex }; 

// console.log(hypergeometric2F1(2,3,4,0.5)) // 2.7289353330396846
// console.log(hypergeometric2F1(1,1,1.5,0.5)) // 1.5707963267238174
